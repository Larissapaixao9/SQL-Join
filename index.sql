...
--Questao 3:
SELECT educations."endDate", school.name, course.name, users.name, users.id FROM educations JOIN courses AS course on educations."courseId"=course.id JOIN schools AS school ON educations."schoolId"=school.id JOIN users on educations."userId"=users.id WHERE users.id=30 AND educations.status='finished' 
